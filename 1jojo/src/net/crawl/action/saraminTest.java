...
public class saraminTest {
	...
	public void Step_02_scraping() throws Exception {
		...
		for (WebElement inner : list) {
			...
			for (WebElement info : companyInfo) {
				...
				// System.out.println("javascript_link :
				// "+javascript_link.getAttribute("href"));
		...
		// String[] noticeKeyword_preferred = {"우대", "자격", "조건"", 지원"지원", "우대", "자격",
		// "조건"};
		// Arrays.sort(noticeKeyword_preferred);
		// System.out.println("noticeKeyword_preferred :
		// "+Arrays.toString(noticeKeyword_preferred));
		// parsing
		int cnt = 0;
		for (String link : listOfRecruitLink) {
			...
			if (table.findElement(By.tagName("recruit_guideline_preferred")).getText() != null) {// case1
																									// 'recruit_guideline_preferred'
																									// 태그가 있음.
				String targetText = table.findElement(By.tagName("recruit_guideline_preferred")).getText();// 이걸 찾는게 제일
																											// 빠르다.
				// 그냥 section에 detail_user_content란 클래스로 명명된 부분 하위 table> thead > th태그 중에 자격|우대
				// 라는 단어가 있는 표 긁어오는 걸로 퉁치기로함 //
			...
			// else if(table.findElement(By.cssSelector(".detail_user_content table
			// th")).getText().contains("지원") ||
			// table.findElement(By.cssSelector(".detail_user_content table
			// th")).getText().contains("자격") ||
			// table.findElement(By.cssSelector(".detail_user_content table
			// th")).getText().contains("우대") ||
			// table.findElement(By.cssSelector(".detail_user_content table
			// th")).getText().contains("조건")) {
			//
			//
			// }
			// else if( table.findElement(By.cssSelector("detail_meta >
			// h3")).getText().contains("지원") ==null ) {//대신 별도의 표로 지원자격이란 detail meta란
			// div하위 h3 class="title_meta" 지원자격/우대조건인 경우.
			//
			//
			// }
			// else if( ) {//이 마저도 없는 이미지 인식은 어려우니 패스하되 표시만 해두자. section에
			// detail_user_content란 클래스로 명명된 부분에 이미지가 있으면 그런 케이스.
			// System.out.println("image!");
			// }

			// //여러칸이라면 모집분야별로 어떻게 할지 고민해봐야함.
			// //근데 아마 사용자의 채용 요건이 명확하지 않다면 이부분은 사실 건들기가 어렵고, 사람인 자체의 검색엔진 문제이기 때문에 그냥
			// 무시하는것이 상책.
			// //tit_section above_table_summary에 (th기준으로 가장 첫번째)
			// if(recruit_guideline_preferred == null) {//case1
			// 'recruit_guideline_preferred' 태그가 없음. 대신 별도의 표로 지원자격이란 detail meta란 div하위 h3
			// class="title_meta" 지원자격/우대조건인 경우.
			//
			//// <div class="detail_meta"><h3 class="title_meta">지원자격</h3><table
			// class="tbl_meta"><colgroup><col style="width:110px;"><col
			// style="width:330px;"><col style="width:110px;"><col
			// style="width:330px;"></colgroup><tbody><tr><th scope="row">경력</th><td
			// colspan="3">신입</td></tr><tr><th scope="row">학력</th><td colspan="3">학력무관 ,
			// 교육수강자 및 개발 가능자</td></tr></tbody></table></div>
			// }
			// if(recruit_guideline_preferred == null) {//case2
			// 'recruit_guideline_preferred' 태그가 없음. 대신 형식은 맞춘 똑같은 표가 존재.
			//// <table class="table_summary" cellspacing="0" cellpadding="0"><colgroup><col
			// width="15%"><col width="30%"><col></colgroup><thead><tr><th
			// scope="col">모집분야</th><th scope="col">담당업무</th><th scope="col">자격요건 및
			// 우대사항</th></tr></thead><tbody><tr><td><p>&nbsp;</p><p>(연구원, 사원,
			// 팀원급)</p></td><td class="left_align">
			//// SI/SM 사업본부 6팀 </td><td class="left_align"><p><b>자격요건</b></p><p>- 학력 : 대졸 이상
			// (4년)</p><p>- 경력 : 신입 </p><p>- 성별 : 무관</p><p>- 모집인원 :
			// 00명</p><br><p><b>필수사항</b></p><p>- 자연계열</p><p>- 전기/전자공학</p><p>-
			// 컴퓨터/시스템공학</p><p>- 공학계열</p><br><p><b>우대사항</b></p><p>- 석사학위 수여자</p><p>- 해당직무
			// 근무경험</p><p>- 인근거주자</p><p>- 문서작성 우수자</p><p>-
			// 정보처리기사</p></td></tr></tbody></table>
			// }
			// if(recruit_guideline_preferred == null) {//case3
			// 'recruit_guideline_preferred' 태그가 없음. 형식도 다르고 전혀 다른 데서 긁어온 표.
			// }
		}
		...
		// driver.findElement(By.xpath("//*[@id='main_top_2']/div[1]/fieldset[1]/div/a/img")).click();//�����ϱ�
		// Ŭ��
		//
		...
		// driver.findElement(By.xpath("//*[@id='title']")).sendKeys("���̺�Ľ����쿡��");
		// //���� �ٽ� �Է�
		//
		//
		//
	...
	public void Step_03_counting_Test() throws Exception {
		...
		// driver.findElement(By.xpath("//img[@alt='�����ܰ�']")).click(); //�����ܰ� ��ư
		// Ŭ��
		//
		// //ī�װ�
		// Thread.sleep(1000);
		// driver.findElement(By.linkText("���丮 ���� ����")).click(); //���丮 ��������
		// Ŭ��
		//
		//
		...
		//
		//
		// dropdown = new Select(driver.findElement(By.id("choose_dir_d2"))); //�ι�°
		// �����ڽ� ������
		...
		// //�����ϱ� â iframe ����Ī
		// driver.switchTo().frame(driver.findElement(By.xpath("//*[@id='editor']")));
		// driver.findElement(By.xpath("//*[@id='au_submit_button2']/div[2]/a[1]/img")).click();//�ӽ�����
		// Ŭ��
		//
		// Thread.sleep(3000);
		//
	...
	public void Step_04_analyze_Test() throws Exception {
		//
		// // �ӽ����� â
		// driver.findElement(By.className("_tempsave_open_close")).click(); //�ӽ�����
		// �� Ŭ��
		//
