...
public class CrawlAddAction implements Action {
	...
	public search_list_Bean categorizer_only_String(search_list_Bean listData, String tableText) {// 이미 자격요건들이 포함된 해당 열의
																									// 문자열. 아마도 테이블인 주제에
																									// 칸이 하나뿐인 경우에도 유효할
																									// 것으로 보임.
	...
	public search_list_Bean categorizer_only_table(WebElement table, search_list_Bean listData) {
		// System.out.println("------tableText start : ------\n" + table.getText() +
		// "\n------table Text end------");
		// if (isContainQualOrPreex(table.findElement(By.tagName("tr")).getText()) ) {//
		// 행에 자격 또는 요건이란 단어가 있는 tr을 찾는다.
		...
		for (WebElement tr : tr_list) {// 행
			...
			if (isThead == true) {// 첫줄에 한해.
				...
				for (WebElement td : td_list) {
					System.out.println("colNo=" + colNo + "/" + (td_list.size() - 1) + ", category=" + category
							+ ", td.getText()" + td.getText());// check
	...
	private search_list_Bean categorizer_only_detail_meta(search_list_Bean listData, WebElement detail_meta) {
		...
		ArrayList<WebElement> tr_list = (ArrayList<WebElement>) table.findElements(By.tagName("tr"));
		for (WebElement tr : tr_list) {// 행
			qualOrPreex += "- " + tr.findElement(By.tagName("th")).getText() + " : "
	...
	public ArrayList<search_list_Bean> Step_02_scraping() throws Exception {
		...
		for (String link : listOfRecruitLink) {
			...
			listData.setSearch_com_No(to);// 일단 사용자당 하나만 부여할까 생각중이다. 생각 같아선 DB연동해서 다른건 싹다 지워버릴 생각이지만 귀찮다. 시간 연동해서 고유번호
											// 지정. 설마 초단위로 동시에 할리는 없다고 생각한다.
			...
			WebElement detail_user_content = driver.findElement(By.className("detail_user_content")); // 일단 이것부터 찾는다.
																										// 대부분 근본이 이쪽을
																										// 따른다.

			try {
				if (!detail_user_content.findElements(By.tagName("recruit_guideline_preferred")).isEmpty()) {// case1
																												// 'recruit_guideline_preferred'
																												// 태그가
																												// 있음.
					String targetText = detail_user_content.findElement(By.tagName("recruit_guideline_preferred"))
							.getText();// 이걸 찾는게 제일 빠르다.
					listData = categorizer_with_recruit_guideline_preferred(listData, targetText);
				} else if (!detail_user_content.findElements(By.className(("table_summary"))).isEmpty()) {// recruit_guideline_preferred는
																											// 없어도
																											// detail_user_content에
																											// th 태그가
																											// 있음(최소한의
																											// 구조를 갖춘 표가
																											// 있는경우).
					System.out.println("table_summary가 있는 경우");// 그중에서도 table_summary가 있는 경우. 제법 많이 나오는 패턴
					listData = categorizer_only_table(detail_user_content.findElement(By.className(("table_summary"))),
							listData);
					...
					listData = categorizer_only_table(detail_user_content.findElement(By.className(("temptb"))),
							listData);// 보통 이런건 배경이 녹색이다. 아마 긁어오는 것으로 보임.
				} else if (!detail_user_content.findElements(By.tagName("table")).isEmpty()) {//
					System.out.println("그나마 참조할만한 테이블이 테이블 안에 있는 경우.");
					WebElement table = detail_user_content.findElement(By.tagName("table")); // 보통 section아래 두겹에서 세겹의
																								// table이 되어 있다.
					...
					WebElement view_content__shadow_section_relay__first = driver
							.findElement(By.cssSelector("div.first"));
					ArrayList<WebElement> detail_meta_list = (ArrayList<WebElement>) view_content__shadow_section_relay__first
							.findElements(By.className("detail_meta"));// detail_meta클래스 중에 관련 정보 찾는다.
					for (WebElement detail_meta : detail_meta_list) {
						// System.out.println("-------detail_meta.getText()-------\n"+detail_meta.getText()+"\n-------end-------");
						if (isContainQualOrPreex(detail_meta.getText())
								&& !detail_meta.findElements(By.className("tbl_meta")).isEmpty()) {// h3에 아마 자격 , 우대 요건
																									// 같은 단어 둘중 하나는 있을
																									// 것이다.
			...
			System.out.println("well crawled ratio = "
					+ (double) (listOfRecruitLink.size() - cntNotDone) / listOfRecruitLink.size());// scrap된 페이지 비율.
		}
		return listOfResult;
	}
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// listOfResult_at_DB = crawldao.getSearch_list( search_com_No );//DB에 놓음 검색결과
			// 읽어온다.
