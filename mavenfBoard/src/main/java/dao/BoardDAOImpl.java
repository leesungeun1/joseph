...
//		sqlsession.commit();		
	...
	/*검색 결과 게시물 수*/
	public int getFindListCount(Map m) throws Exception{
		/*		
 		Map m = new HashMap();
		m.put("page", page);
		...
		*/
		...
		getSession();
		int count=0;		
			count=((Integer)sqlsession.selectOne("Test.boardfind_cnt",m)).intValue();
	...
	/*검색 결과 페이징 목록*/
	public List<BoardBean> getFindBoardList(Map m) throws Exception {
		getSession();
		List<BoardBean> list = sqlsession.selectList("Test.board_find", m);
